...
# Student1 name: Roy Zarum
# Student1 ID:
# Student2 name: Eyal Shpater
# Student2 ID:
...
def calculate_distance(vector1, vector2, distance_type='euclidean'):
    ...
    if distance_type == 'euclidean':
        distance = np.linalg.norm(vector1 - vector2)  # hint:use np.linalg.norm
    elif distance_type == 'manhattan':
        distance = np.sum(np.abs(vector1 - vector2))
    elif distance_type == 'cosine':
        dot_product = np.dot(vector1, vector2)
        vector1norm = np.linalg.norm(vector1)
        vector2norm = np.linalg.norm(vector2)
        cosine_similarity = dot_product / (vector1norm * vector2norm)
...
def test_calculate_distance():
    ...
    def calculate_distance_matrix(vectors_list, distance_type='euclidean'):
        ...
        distance_matrix = np.inf * np.ones((num_vectors, num_vectors))

        for i in range(num_vectors):
            for j in range(i + 1, num_vectors):
                distance_matrix[i, j] = calculate_distance(vectors_list[i], vectors_list[j], distance_type)
...
def k_nearest_neighbors(train_set, k=3, distance_type='euclidean'):
    ...
    def classify(test_sample):
        ...
        distances = [calculate_distance(test_sample, train_vector, distance_type) for train_vector in train_vectors]
        sorted_indices = np.argsort(distances)[:k]
        k_nearest_labels = train_set['label'].iloc[sorted_indices].tolist()

        majority_vote = max(set(k_nearest_labels), key=k_nearest_labels.count, default=k_nearest_labels[0])
...
if __name__ == '__main__':
    ...
    knn_classifier = k_nearest_neighbors(train_set, k=1, distance_type='cosine') # Choose the parameeters that returned the best results.
